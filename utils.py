...
def rotate_around_axis(v1, axis, angle):
    return transform(v1, rotation_mat(q(axis, angle)))
...
class PID:
    def __init__(self, kp=1, ki=0, kd=1, sd=0):
        self.ep = True
        self.ei = True
        self.ed = True
        self.kp = kp
...
class PIDn():
    def __init__(self, n, **kwargs):
        self.controllers = [PID(**kwargs) for i in range(n)]
        
    def update(self, error, dt):
        assert len(error) == len(self.controllers)
        res = np.zeros(len(error))
        for i in range(len(error)):
            res[i] = self.controllers[i].update(error[i], dt)
        return res
    
    def set(self, k, v):
        assert k in ['ep', 'ei', 'ed', 'kp', 'ki', 'kd', 'sd', 'integral_limit']
        for c in self.controllers:
            c.__setattr__(k, v)
    
    def get(self, k):
        return np.array([p.__getattribute__(k) for p in self.controllers], dtype=float)
